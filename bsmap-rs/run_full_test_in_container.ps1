# P-series complete test - single container
...
# Create single script that runs everything
$fullScript = @"
#!/bin/bash
set -e
...
$fullScript | Out-File -FilePath "$WORK_DIR\full_test.sh" -Encoding ASCII

# Run everything in one container
Write-Host "[Running] All tests in single container..."
Write-Host ""
...
docker run --rm -it `
    ...
    --memory=20g `
    --cpus=4 `
    ubuntu:22.04 bash /workspace/bsmap-rs/full_test.sh 2>&1 | Tee-Object -FilePath "$WORK_DIR\benchmark\results\full_test_output.log"
