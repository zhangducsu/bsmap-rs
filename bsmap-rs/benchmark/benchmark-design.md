...
## 5. 比对参数 (已更新 - 2026-05-17)

### 5.1 索引构建参数 (修正版)

```bash
# 原版 BSMAP - WGBS 模式
bsmap -a ref.fa -o index_wgbs.bsi -s 16 -I 4

# 原版 BSMAP - RRBS 模式
bsmap -a ref.fa -o index_rrbs.bsi -s 12 -I 4 -D C-CGG

# bsmap-rs - WGBS 模式
cargo run --release -p bsmap -- index -d ref.fa -o index_wgbs.bsi -s 16 -I 4

# bsmap-rs - RRBS 模式
cargo run --release -p bsmap -- index -d ref.fa -o index_rrbs.bsi -s 12 -I 4 -D C-CGG
```
...
```bash
# 原版 BSMAP
bsmap -a reads_1.fq -d ref.fa -o output.sam -s 16 -v 0.08 -I 4 -p 1

# bsmap-rs
cargo run --release -p bsmap -- align -a reads_1.fq -d ref.fa -o output.sam -s 16 -v 0.08 -I 4 -p 1
```

**双端 (Example 2, 4, 5, 6)**:
```bash
# 原版 BSMAP
bsmap -a reads_1.fq -b reads_2.fq -d ref.fa -o output.sam -s 16 -v 0.08 -I 4 -p 1

# bsmap-rs
cargo run --release -p bsmap -- align -a reads_1.fq -b reads_2.fq -d ref.fa -o output.sam -s 16 -v 0.08 -I 4 -p 1
```
...
| 参数 | 错误值 | 正确值 | 说明 |
|------|--------|--------|------|
| 种子大小 | `-v 16` | `-s 16` | 种子大小参数是 -s 不是 -v |
| 错配阈值 | `-i 4` | `-I 4` | 错配阈值参数是 -I (大写) 不是 -i |
| RRBS 酶切 | `-e MspI` | `-D C-CGG` | RRBS 模式使用 -D 指定酶切位点 |
| 子命令 | 无 | `align` | bsmap-rs 需要显式指定 `align` 子命令 |
...
**SAM 过滤规则**:
```bash
# 过滤掉 @HD, @SQ, @PG 行后逐行 diff
grep -v "^@HD\|^@SQ\|^@PG" sam1 > sam1.filtered
grep -v "^@HD\|^@SQ\|^@PG" sam2 > sam2.filtered
...
测试结果记录到 CSV 文件:
```csv
example,tool,dataset,mode,reads,time_wall,time_user,time_sys,mem_max_rss,index_size,alignment_rate,unique_pairs,multi_pairs
```

---

## 7. 测试流程 (已更新 - 2026-05-17)

### 阶段 1: 环境准备
1. 确认工具链 (Rust、原版 BSMAP、Sherman、RRBSsim)
2. 安装 Python 依赖 (`pip install pyfaidx`)
3. **预编译 bsmap-rs release 版本** (重要：避免每次运行重新编译)
...
对每个 Example 依次:
1. 原版 BSMAP 比对 (使用解压后的临时文件)
2. bsmap-rs 比对 (使用解压后的临时文件)
3. SAM 输出对比
4. 记录时间/内存
...
| **进程替换不工作** | 无法读取压缩数据 | **先解压到临时文件再读取** |
| **参数错误** | 程序崩溃 | **使用修正后的参数** (-s/-I/-D, 明确 align 子命令) |
| **重复编译 Rust** | 测试时间过长 | **预编译 release 版本** |
| 内存接近上限 | 可能 OOM | 监控 RSS，必要时降低规模或提升内存到16GB |
...
|------|------|------|
| 2026-05-15 | 1.0 | 初始设计 |
| 2026-05-17 | 1.1 | 根据测试经验修正参数，添加预编译和临时文件步骤 |
