...
# bsmap-rs vs BSMAP C++ 对比测试 - 阶段3-6执行脚本
# ==================================================
...
echo "=========================================="
echo "开始执行阶段3-6 benchmark测试"
echo "=========================================="
date
echo ""

# 创建必要目录
mkdir -p index results report
...
/usr/bin/time -v /workspace/bsmap-original/bsmap-2.90/bsmap \
    -a data/chr22_tail_1M.fa \
    -o index/bsmap_wgbs.bsi \
    -s 16 -v 16 -I 4 2>&1 | tee index/bsmap_wgbs_build.log || {
    echo "原版 WGBS 索引构建失败，继续执行..."
}
...
/usr/bin/time -v /workspace/bsmap-original/bsmap-2.90/bsmap \
    -a data/chr22_tail_1M.fa \
    -o index/bsmap_rrbs.bsi \
    -s 16 -v 16 -I 4 -D C-CGG 2>&1 | tee index/bsmap_rrbs_build.log || {
    echo "原版 RRBS 索引构建失败，继续执行..."
}
...
/usr/bin/time -v cargo run --release -p bsmap -- index \
    ...
    -o index/bsmaprs_wgbs.bsi \
    -s 16 -I 4 2>&1 | tee index/bsmaprs_wgbs_build.log || {
...
/usr/bin/time -v cargo run --release -p bsmap -- index \
    -d data/chr22_tail_1M.fa \
    -o index/bsmaprs_rrbs.bsi \
    -s 16 -I 4 -D C-CGG 2>&1 | tee index/bsmaprs_rrbs_build.log || {
...
echo "    运行原版 BSMAP..."
/usr/bin/time -v /workspace/bsmap-original/bsmap-2.90/bsmap \
    -a <(gunzip -c data/rrbs/rrbssim/ex3_se75_10x.1.fq.gz) \
    -d data/chr22_tail_1M.fa \
...
echo "    运行原版 BSMAP..."
/usr/bin/time -v /workspace/bsmap-original/bsmap-2.90/bsmap \
    -a <(gunzip -c data/rrbs/rrbssim/ex4_pe150_10x.1.fq.gz) \
    -b <(gunzip -c data/rrbs/rrbssim/ex4_pe150_10x.2.fq.gz) \
    -d data/chr22_tail_1M.fa \
...
echo "    运行原版 BSMAP..."
/usr/bin/time -v /workspace/bsmap-original/bsmap-2.90/bsmap \
    -a <(gunzip -c data/rrbs/rrbssim/ex6_pe150_20x.1.fq.gz) \
    -b <(gunzip -c data/rrbs/rrbssim/ex6_pe150_20x.2.fq.gz) \
    -d data/chr22_tail_1M.fa \
...
# 替换日期
sed -i "s/\$(date +%Y-%m-%d)/$(date +%Y-%m-%d)/g" report/benchmark_report.md
sed -i "s/\$(date)/$(date)/g" report/benchmark_report.md
...
echo "  - benchmark_report.md: $WORK_DIR/report/benchmark_report.md"
