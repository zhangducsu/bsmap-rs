...
2026-06-27 服务器 `d761fe4` 追加验证：Docker 内 GitHub fetch 超时后，使用本地 Git bundle 增量同步到容器，checkout 干净位于 `d761fe4`；release binary SHA256 为 `3a4e2e96f0d9ac120f254dca1c5d91dacdcf818e6e7bc33224b10ff9154e88e1`。10K SE `SSH1_PROFILE_RRBS=stage` run 路径为 `/workspace/benchmark_results/ssh1/20260627T155449Z-71203/summary.json`，Rust wall 1.41s、RSS 893,040 KiB、mapped 2,423；C++ wall 66.77s、RSS 2,056,756 KiB、mapped 2,423；字段 diff 仍为 0。stage profile 只包含 `read_seconds=0.011400`、`prepare_seconds=0.013688`、`align_seconds=1.300677`、`write_seconds=0.023240`，确认 10K 慢点集中在 align core，full SE 后续应先优化候选/mismatch/早停路径。

2026-06-27 负收益/低收益尝试：`390c0c2 perf: streamline RRBS candidate traversal` 将 RRBS logical bucket 的 iterator 链改为手写 circular scan，并把 `N` 计数从每个 segment hoist 到每条 read-chain。验证通过：本地 `cargo check/test/build` 通过；服务器 10K SE Rust/C++ mapped 均为 2,423，字段 diff 为 0，SAM SHA 为 `420e34a3fa39086effbff8341cde5bacf90fde9bf57a32b39e0cb48eeedd9ad0`。但 Rust-only A/B 五轮结果 `/workspace/benchmark_results/ssh1/ab_rrbs_traversal_20260627T160717Z_72464` 显示 `d761fe4` median 1.41s、`390c0c2` median 1.40s，收益约 0.7%，低于 3% 保留门槛且增加代码复杂度；已用 `fb66e26` revert，不作为 SSH1 保留优化。
